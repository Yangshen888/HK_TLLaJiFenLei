      ...
      >
        ...
        >
          ...
          <template slot-scope="{ row, index}" slot="action">
            ...
            <Tooltip placement="top" content="称重记录详情" :delay="1000" :transfer="true">
              <Button
                v-can="'show'"
                type="warning"
                size="small"
...
export default {
      ...
      stores: {
        car: {
          ...
          columns: [
            ...
            { title: "当前状态",width: 100, key: "state" },
            { title: "易腐垃圾比例",width: 100, key: "rottenRubbishPercent" },                      
            { title: "收运时间",width: 200, key: "addTime" },  
                          {
              title: "操作",
              align: "center",
              width: 150,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel1.selection.map(x => x.grabageWeighingRecordUuid);
    } //删除
  },
  methods: {
    //获取垃圾箱房收运数据
    ...
    handleDetail(row) {
      ...
      let data = { guid: row.garbageRoomUuid.toString() ,time:row.addTime};
      //console.log(data);
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.grabageWeighingRecordUuid);
    },
    doDelete(ids) {
      if (!ids) {
