...
import { setSignInfo, addSignInfo, getSignInfo, delSignInfo, getInfo, key } from '@/global/index.js';
import {
	doClockIn,
	doClockBack,
	message
...
export default {
	...
	methods: {
		...
		clickSign() {
			...
			var a = getSignInfo(2);
			if (a != undefined) {
				addSignInfo(getInfo(this.signInfo), a,2);
			} else {
				setSignInfo(getInfo(this.signInfo),2);
			}
			// setTimeout(function() {
				uni.hideLoading();
				var sign = getSignInfo(2).main;
				// console.log(sign)
				...
				if (that.isAm === false) {
					...
					that.signModel.PmendPlace=that.pmSign.address;
